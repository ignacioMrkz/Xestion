...
<div class="container">
	...
	@endif
	{!!Form::open(['url'=>'monitor/avaliacion-monitor'])!!}
	<div class="row">
		<div class="input-field col s12">
			<h5>Actividade:</h5>
			<select name="actividade_id">
				<option value="" disabled selected="selected">Escolle unha actividade</option>
				@foreach($actividades as $actividade)
				<option value="{!!$actividade->id!!}" {{ old('actividade_id') == $actividade->id ? ' selected' : '' }}>{!!$actividade->nome!!}</option>
				@endforeach
			</select>
		</div>
	</div>
	<div class="row">
		...
		<div class="input-field col s12 l4">
			...
			<select name="concello">
				<option value="" disabled selected="selected">Escolle unha cidade</option>
				@foreach($pontevedra as $cidade)
				<option value="{!!$cidade!!}"  {{ old('concello') == $cidade ? ' selected' : '' }}>{!!$cidade!!}</option>
				@endforeach
			</select>
		</div>
	</div>
